...
/**
 * A class for reading Microsoft Excel Spreadsheets (.xls).
 * Vendored MIT reader — Sergey Shuchkin SimpleXLS / Vadim Tkachenko Spreadsheet_Excel_Reader.
 */
...
// if ( $v['type'] === self::TYPE_RK || $v['type'] === self::TYPE_RK2 ||
 ...
 $s .= ' ';
 foreach ($r as $c) {
 $s .= ' ' . ( $c === '' ? '&nbsp' : htmlspecialchars($c, ENT_QUOTES) ) . ' ';
 }
 $s .= " \r\n";
 }
 $s .= ' ';
