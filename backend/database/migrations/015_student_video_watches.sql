-- Per-video watch status marked by the student (manual checkbox).
SET NAMES utf8mb4;
...
CREATE TABLE IF NOT EXISTS student_video_watches (
    ...
    CONSTRAINT fk_svw_student FOREIGN KEY (student_id) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT fk_svw_resource FOREIGN KEY (resource_id) REFERENCES lecture_resources(id) ON DELETE CASCADE
) ENGINE=InnoDB;
